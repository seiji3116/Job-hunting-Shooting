...
//bool KdAnimationData::Node::NextAnimationInterpolateTranslations(Math::Vector3& result, const Math::Matrix prev, float time)
//{
//	return true;
//}
//
//bool KdAnimationData::Node::NextAnimationInterpolateRotations(Math::Quaternion& result, const Math::Matrix prev, float time)
//{
//	return true;
//}
//
//bool KdAnimationData::Node::NextAnimationInterpolateScales(Math::Vector3& result, const Math::Matrix prev, float time)
//{
//	//if (m_scales.size() == 0)return false;
//
//	//// キー位置検索
//	//UINT keyIdx = BinarySearchNextAnimKey(m_scales, 0);
//	// 
//	//auto& next = m_scales[keyIdx];		// 次のキー
//
//	//// 補間
...
//void KdAnimationData::Node::NextAnimationInterpolate(Math::Matrix& rDst, float time)
//{
//	// ベクターによる拡縮補間
//	bool isChange = false;
//	Math::Matrix scale;
//	Math::Vector3 resultVec;
//	if (NextAnimationInterpolateScales(resultVec, rDst, time))
//	{
//		scale = scale.CreateScale(resultVec);
//		isChange = true;
//	}
...
//	Math::Matrix rotate;
//	Math::Quaternion resultQuat;
//	if (NextAnimationInterpolateRotations(resultQuat, rDst, time))
//	{
//		rotate = rotate.CreateFromQuaternion(resultQuat);
//		isChange = true;
//	}
...
//	// ベクターによる座標補間
//	Math::Matrix trans;
//	if (NextAnimationInterpolateTranslations(resultVec, rDst, time))
//	{
//		trans = trans.CreateTranslation(resultVec);
//		isChange = true;
//	}
...
void KdAnimator::AdvanceTime(std::vector<KdModelWork::Node>& rNodes, float speed)
{
	...
	//if (m_spAnimation == m_spNextAnimation)
	//{
	//	// 全てのアニメーションノード（モデルの行列を補間する情報）の行列補間を実行する
	//	for (auto& rAnimNode : m_spAnimation->m_nodes)
	//	{
	...
	//else
	//{
	//	ChengeAnimation(rNodes, speed);
	//}
}

//void KdAnimator::ChengeAnimation(std::vector<KdModelWork::Node>& rNodes, float speed)
//{
//	// 全てのアニメーションノード（モデルの行列を補間する情報）の行列補間を実行する
//	for (auto& rAnimNode : m_spNextAnimation->m_nodes)
//	{
//		// 対応するモデルノードのインデックス
//		UINT idx = rAnimNode.m_nodeOffset;
//
...
//
//		// アニメーションデータによる行列補間
//		rAnimNode.NextAnimationInterpolate(rNodes[idx].m_localTransform, m_changeTime);
//
//		prev = rNodes[idx].m_localTransform;
//	}
//
...
//	if (m_changeTime >= 1.f)
//	{
//		// 次のアニメーションのために最初のフレームにセットする
//		m_changeTime = 0.0f;
//		m_spAnimation = m_spNextAnimation;	// アニメーションの切り替え
//	}
//}
