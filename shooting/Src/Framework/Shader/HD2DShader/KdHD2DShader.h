...
public:
	...
	// 陰影は付けたいがUnLitの後に描画したいオブジェクト等を描画する前後に行う
	void BeginDrawUnlitWithTransparency();
	void EndDrawUnlitWithTransparency();

	// 陰影は付けたいがUnLitの後に描画したいオブジェクト等を描画する前後に行う(スキンメッシュ対応)
	void BeginDrawUnlitWithTransparency_SkinMesh();
	void EndDrawUnlitWithTransparency_SkinMesh();
